...
describe('urlDetector - Property-Based Tests', () => {
  ...
  describe('プロパティ 2: 除外条件の適用', () => {
    it('should not detect URLs inside <a> tags', () => {
       ...
       */
      fc.assert(
        fc.property(fc.webUrl(), fc.string(), (url, linkText) => {
          // リンクタグ内にURLを配置
    ...
    it('should not detect own site URLs (hidetaka.dev)', () => {
       ...
       */
      fc.assert(
        fc.property(fc.constantFrom('http', 'https'), fc.string(), (protocol, path) => {
          // 自サイトURLを生成
  ...
  describe('プロパティ 3: 複数URLの完全な検出', () => {
    it('should detect all independent URLs without duplicates', () => {
       ...
       */
      fc.assert(
        fc.property(fc.array(fc.webUrl(), { minLength: 1, maxLength: 20 }), (urls) => {
          // 重複を除去（reduce + Mapパターン）
          const uniqueUrls = Array.from(
            urls
              .reduce((map, url) => {
    ...
    it('should preserve order of first occurrence when deduplicating', () => {
       ...
       */
      fc.assert(
        fc.property(fc.array(fc.webUrl(), { minLength: 2, maxLength: 10 }), (urls) => {
          // 重複を除去（reduce + Mapパターン）
          const uniqueUrls = Array.from(
            urls
              .reduce((map, url) => {
