...
function isPortfolioType(types: string[] | undefined): boolean {
  if (!types) return false
  return (
    types.includes('oss_contribution') ||
    types.includes('community_activities') ||
    types.includes('guest_posts')
  )
}
...
export async function generateStaticParams() {
  ...
  return projects
    .filter((project) => isPortfolioType(project.project_type))
    .map((project) => ({
      slug: project.id,
    }))
}
...
export default async function PortfolioDetailPage({
  ...
  const project = await microCMS
    .listAllProjects()
    .then((projects) => projects.find((p) => p.id === slug && isPortfolioType(p.project_type)))
              ...
              <ol className="flex items-center space-x-2">
                <li>
                  <div className="flex items-center text-sm">
                    <a href="/portfolio" className="font-medium text-gray-500 hover:text-gray-900">
                      Portfolio
                    </a>
                    <svg
                      viewBox="0 0 20 20"
