...
function isProductType(types: string[] | undefined): boolean {
  if (!types) return false
  return types.includes('applications') || types.includes('books') || types.includes('owned_oss')
}
...
export async function generateStaticParams() {
  ...
  return projects
    .filter((project) => isProductType(project.project_type))
    .map((project) => ({
      slug: project.id,
    }))
}
...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const project = await microCMS
    .listAllProjects()
    .then((projects) => projects.find((p) => p.id === slug && isProductType(p.project_type)))
