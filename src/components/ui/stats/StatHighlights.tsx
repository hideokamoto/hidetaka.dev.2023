...
import type { WritingStreak } from '@/libs/stats/writingActivity'
 ...
 * streak が取れないとき（index.json 未取得）は投稿数カードだけを出す。
 * レイアウトは StatCardGrid に委譲し、値の整形と i18n のみを担う。
 */
export default function StatHighlights({ total, streak, lang }: Props) {
  const isJa = lang === 'ja'
    ...
    highlights.push(
      {
        ...
        value: weeksUnit(streak.currentWeeks),
      },
      {
        label: isJa ? '最長の週連続記録' : 'Longest weekly streak',
        value: weeksUnit(streak.longestWeeks),
      },
    )
  }

  return <StatCardGrid items={highlights} columns={streak ? 3 : 1} />
}
